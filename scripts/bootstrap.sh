...
plan_apps_desc() {
  local n; n="$(resolve_apps | sed '/^$/d' | grep -c . || true)"
  [[ "${n:-0}" -eq 0 ]] && { echo "none"; return; }
  echo "$n selected"
}

install_apps() {
  local apps count; apps="$(resolve_apps)"; count="$(printf '%s\n' "$apps" | sed '/^$/d' | grep -c . || true)"
  ...
  { echo "# Generated by macstrap on $(date +%F). Do not edit."
    printf '%s\n' "$apps" | sed '/^$/d' | sed 's/.*/cask "&"/'; } > "$gen/Brewfile.apps.local"
